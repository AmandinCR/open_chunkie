...
%% === Flux of TRUE field through the torus hole (no edits needed) ===
% Uses your existing variables from magnetostatics3.m:
%   Rin, origin, m0, all_modes, a1, a2, srcQ1p, srcQ1m, srcQ2p, srcQ2m
% and the same "true field" convention you used to build g0:
%   H_true_physical = -grad(phi_true)
%
% Flux is through the flat disk z=0, 0<=r<=Rin, with n_A = +zhat.

Nr_flux = 50;                 % increase if you want
Rin = min(src(1,:));
rflux = linspace(0.01, Rin, Nr_flux+1).';
dr = rflux(2)-rflux(1);

Hz_true = zeros(size(rflux));   % this is d(phi_true)/dz from green_modal
Hz = zeros(size(rflux));   % this is d(phi_true)/dz from green_modal

for ii = 1:numel(rflux)
    targF = [rflux(ii); 0.0];

    [~, gradQ1] = chnk.axissymlap2d.green_modal(srcQ1, targF, origin, m0, all_modes);
    [~, gradQ2] = chnk.axissymlap2d.green_modal(srcQ2, targF, origin, m0, all_modes);
    
    % gradient of phi_true (z-component)
    Hz_true(ii) = gradQ1(1,1,3) - gradQ2(1,1,3);
    ...
    tinfo_z.r = targF;
    tinfo_z.n = [0;1];
    Hz(ii) = -chunkerkerneval(chnkr, Sp0, sigma0, tinfo_z, opts_eval);
end

% Flux through hole with n_A = +zhat
integrand_flux = Hz_true .* rflux;
...
integrand_flux = Hz .* rflux;
flux_true_hole = 2*pi * dr * (0.5*integrand_flux(1) + sum(integrand_flux(2:end-1)) + 0.5*integrand_flux(end));

fprintf('\n=== Solution flux through torus hole (disk z=0, n_A=+zhat) ===\n');
fprintf('Flux_true = %.16e\n', flux_true_hole);
